                ...
                database.getReference().child("categories").child(getIntent().getStringExtra("key")).child("sets").child(id).setValue("setId").addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()){
                         ...
                         Toast.makeText(SetsActivity.this , "Somethingmwent wrong" , Toast.LENGTH_LONG).show();
                        }
                ...
                new AlertDialog.Builder(SetsActivity.this , R.style.Theme_AppCompat_Light_Dialog_Alert)
                        ...
                        .setPositiveButton("Delete", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                loadingDialog.show();
                                         reference.child("SETS").child(categoryName).child("questions")
                                                 .orderByChild("setNo").equalTo(setNo).addListenerForSingleValueEvent(new ValueEventListener() {
                                             @RequiresApi(api = Build.VERSION_CODES.KITKAT)
                                             @Override
                                             public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                 for (DataSnapshot dataSnapshot1 : snapshot.getChildren()){
                                                     String id = dataSnapshot1.getKey();
                                                     reference.child("SETS").child(categoryName).child("questions").child(id).removeValue();
                                                 }
                                                 gridAdapter.sets.size()--;
                                                 loadingDialog.dismiss();
                                                 gridAdapter.notifyDataSetChanged();
                                             }
                                             @Override
